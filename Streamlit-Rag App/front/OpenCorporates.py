...
class OpenCorporates:
    ...
    @staticmethod
    def call_api(jurisdiction: str, company_number: str, token: str): #calls api based on company number
    ...
    @staticmethod
    def pad_cik(cik): # function that pads cik
        """_summary_
    ...
    def updated_search_api(self, token: str, jurisdiction_code: str, company_name: str, cik = ''): #calls api to search for company
        if cik == '': #no cik input go straight to loose search
            return self.loose_search_api(company_name, jurisdiction_code, token)
        ...
        if data['results']['total_count'] == 0: # if no company is returned pad cik with zeros
            ...
            statements_url = 'https://api.opencorporates.com/v0.4/companies/search?q='+ '' + '&jurisdiction_code='+ jurisdiction_code + '&identifier_uids='+ padded_cik + '&per_page=1&page=1&order=score&amp;api_token=' + token
            r = requests.get(url = statements_url, 
                             timeout=5)
            data = r.json()
        ...
        for i in keep_items:
            if i in list(data.keys()):
                if i == 'registered_address':
                    if bool(data[i]) == True and data[i] != 'None':
                        ...
                        continue
                if i == 'agent_address':
                    if clean_data['has_registered_address'] == False:
                        if type(data[i]) == dict:
                            updated_data = ', '.join(data[i].values())
                            clean_data.update({'registered_address': updated_data})
                        else:
